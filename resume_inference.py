...
def run_all():
    ...
    # Dynamically find the Level mapping
    video_to_level = {}
    ...
    for _, row in df_videos.iterrows():
        ...
        # Robust skip logic based entirely on checkpoint state
        if vid_id in processed_vids:
            print(f"Skipping {vid_id} (Already in checkpoint)")
            continue
        ...
        for start_sec, end_sec, max_motion in candidates:
            ...
            # RESOLUTION CAPPING TO PREVENT OOM
        ...
        print(f"Video {vid_id} complete. Progress saved to {CHECKPOINT_FILE}.")
        ...
        print("Runtime:")
        print(f"    Total: {vid_total_ms:.1f}ms | VLM Total: {sum(vlm_durations):.1f}ms")
    ...
    print("Saved final output to predictions.json")
    print("="*50)
