...
if __name__ == "__main__":
    ...
    # 6. Test Sampler Distribution
    print("\n--- Testing Sampler Distribution ---")
    ...
    print(f"Class distribution over {samples_collected} drawn samples:")
    
    # Calculate perfect balance target for 12 classes
    ideal_percentage = 100.0 / len(train_dataset.classes)
    print(f"(Ideal balance per class is ~{ideal_percentage:.1f}%)\n")
    
    for cls in train_dataset.classes:
        count = class_counts.get(cls, 0)
        percentage = (count / samples_collected) * 100
        print(f"{cls.ljust(35)}: {count} samples ({percentage:.1f}%)")